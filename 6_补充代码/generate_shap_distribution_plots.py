...
# 训练模型并计算SHAP值
def train_model_and_get_shap(time_of_day, X_train, y_train, X_test, y_test):
    print(f"\n--- {time_of_day}模型训练与SHAP计算 ---")
...
# 生成SHAP分布图
def generate_shap_distribution_plots(time_of_day, shap_values, X_test, y_test):
    print(f"\n--- 生成{time_of_day}SHAP分布图 ---")
    
    if time_of_day == "早高峰":
        save_dir = early_peak_dir
        color = early_peak_color
    else:
    ...
    for i, feature in enumerate(X_test.columns):
        ...
        plt.title(f'{time_of_day} - {feature_display_name} SHAP分布图', fontsize=16, fontweight='bold')
        ...
        plt.savefig(os.path.join(save_dir, f"{time_of_day}_{clean_feature}_SHAP分布图.png"), 
                   dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"已生成: {time_of_day}_{clean_feature}_SHAP分布图.png")

# 生成SHAP值与目标值的关系图
def generate_shap_target_relationship_plots(time_of_day, shap_values, X_test, y_test):
    print(f"\n--- 生成{time_of_day}SHAP值与目标值关系图 ---")
    ...
    for i, feature in enumerate(X_test.columns):
        ...
        ax1.set_title(f'{time_of_day} - {feature_name_mapping.get(feature, feature)} SHAP分析')
        ...
        ax2.set_title(f'{time_of_day} - {feature_name_mapping.get(feature, feature)} 与碳排放关系')
        ...
        # 保存图片
        clean_feature = feature.replace(' ', '_').replace('(', '').replace(')', '').replace('/', '')
        plt.savefig(os.path.join(save_dir, f"{time_of_day}_{clean_feature}_SHAP目标值关系图.png"), 
                   dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"已生成: {time_of_day}_{clean_feature}_SHAP目标值关系图.png")

# 生成综合SHAP分析图
def generate_comprehensive_shap_analysis(time_of_day, shap_values, X_test, y_test):
    print(f"\n--- 生成{time_of_day}综合SHAP分析图 ---")
    ...
    # 设置总标题
    fig.suptitle(f'{time_of_day} - 主要特征SHAP分布综合分析', fontsize=16, fontweight='bold')
    
    # 调整布局
    plt.tight_layout()
    
    # 保存图片
    plt.savefig(os.path.join(save_dir, f"{time_of_day}_综合SHAP分析图.png"), 
               dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"已生成: {time_of_day}_综合SHAP分析图.png")

# 生成SHAP值分布直方图
def generate_shap_distribution_histograms(time_of_day, shap_values, X_test):
    print(f"\n--- 生成{time_of_day}SHAP值分布直方图 ---")
    ...
    # 设置总标题
    fig.suptitle(f'{time_of_day} - 主要特征SHAP值分布直方图', fontsize=14, fontweight='bold')
    
    # 调整布局
    plt.tight_layout()
    
    # 保存图片
    plt.savefig(os.path.join(save_dir, f"{time_of_day}_SHAP值分布直方图.png"), 
               dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"已生成: {time_of_day}_SHAP值分布直方图.png")

# 主函数
def main():
    print("=== 基于SHAP值生成单一变量与目标值碳减排量的分布图 ===")
    ...
    model_early, explainer_early, shap_values_early, X_early_test, y_early_test = \
        train_model_and_get_shap("早高峰", X_early_train, y_early_train, X_early_test, y_early_test)
    
    # 生成早高峰SHAP分析图
    generate_shap_distribution_plots("早高峰", shap_values_early, X_early_test, y_early_test)
    generate_shap_target_relationship_plots("早高峰", shap_values_early, X_early_test, y_early_test)
    generate_comprehensive_shap_analysis("早高峰", shap_values_early, X_early_test, y_early_test)
    generate_shap_distribution_histograms("早高峰", shap_values_early, X_early_test)
    ...
    model_late, explainer_late, shap_values_late, X_late_test, y_late_test = \
        train_model_and_get_shap("晚高峰", X_late_train, y_late_train, X_late_test, y_late_test)
    
    # 生成晚高峰SHAP分析图
    generate_shap_distribution_plots("晚高峰", shap_values_late, X_late_test, y_late_test)
    generate_shap_target_relationship_plots("晚高峰", shap_values_late, X_late_test, y_late_test)
    generate_comprehensive_shap_analysis("晚高峰", shap_values_late, X_late_test, y_late_test)
    generate_shap_distribution_histograms("晚高峰", shap_values_late, X_late_test)
    ...
    # 生成分析报告
    print("\n=== 分析说明 ===")
    print("1. SHAP分布图：显示每个特征值与其SHAP值的关系，反映特征对模型预测的影响")
    print("2. SHAP目标值关系图：展示SHAP值、特征值与碳排放三者之间的关系")
    print("3. 综合SHAP分析图：汇总主要特征的SHAP分布，便于对比分析")
    print("4. SHAP值分布直方图：显示每个特征SHAP值的分布特征和统计特性")
    print("\n色系说明：")
    print(f"- 早高峰：{early_peak_color}（深红色系）")
    print(f"- 晚高峰：{late_peak_color}（深蓝色系）")
