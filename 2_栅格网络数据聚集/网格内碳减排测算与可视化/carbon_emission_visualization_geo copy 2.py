...
def visualize_carbon_emission_with_fishnet(emission_data, time_period, overall_max=None):
    ...
    try:
        ...
        # 自定义稀疏刻度分布，减少刻度数量，确保包含完整的0到顶点值范围
        if tick_ceil <= 50:
            bounds = [0, 6, 12, 18, 25, 32, 40, tick_ceil]
        elif tick_ceil <= 100:
            bounds = [0, 6, 12, 18, 25, 35, 45, 60, 80, tick_ceil]
        else:  # 早高峰情况
            bounds = [0, 6, 12, 18, 25, 35, 45, 60, 80, 100, 120, tick_ceil]
        ...
        # 设置颜色条属性
        cbar.set_label('碳减排量 (kg)', fontsize=16)
        cbar.ax.tick_params(labelsize=16)  # 使用ax属性设置刻度大小
        cbar.ax.set_title('碳减排量 (kg)', fontsize=16, pad=10)  # 使用ax属性设置标题
