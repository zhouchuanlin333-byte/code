...
def plot_road_density_shap_scatter(period, X, shap_values):
    """专门绘制道路密度SHAP值分布散点图"""
    print(f"\n--- 生成{period}道路密度SHAP分布散点图 ---")
    
    # 获取原始数据
    df_original = load_original_data(period)
    
    target_var = "道路密度 (KM/KM²)"
    display_name = "道路密度"
    ...
    print(f"有效样本数量: {len(original_feature_values)}")
    print(f"道路密度范围: [{np.min(original_feature_values):.3f}, {np.max(original_feature_values):.3f}] KM/KM²")
...
def generate_road_density_shap_plot():
    """生成道路密度SHAP分布散点图"""
    period = "早高峰"
    ...
    try:
        ...
        # 生成道路密度散点图
        plot_road_density_shap_scatter(period, X, shap_values)
...
if __name__ == "__main__":
    generate_road_density_shap_plot()