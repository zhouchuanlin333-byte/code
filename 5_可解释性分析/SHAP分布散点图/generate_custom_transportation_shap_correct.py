...
def plot_custom_transportation_shap_scatter():
    ...
    # 绘制散点图 - 使用所有原始数据点，不进行任何过滤
    # 设置vmin和vmax来控制颜色映射范围，但不影响散点位置
    ...
    # 保存图片
    filename = f'{period}_{display_name}_SHAP分布散点图_自定义坐标轴.png'
    filepath = os.path.join(output_dir, filename)
    plt.savefig(filepath, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"自定义{period} {display_name} SHAP分布散点图已保存：{filepath}")
